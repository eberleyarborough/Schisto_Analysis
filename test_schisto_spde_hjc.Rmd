...
if(!require(inlabru)) install.packages("inlabru")
...
if(!file.exists(file.path(file_path, github, "figures"))) file.create(file.path(file_path, github, "figures"))
if(!file.exists(file.path(file_path, github, "tables"))) file.create(file.path(file_path, github, "tables"))
if(!file.exists(file.path(file_path, github, "inla.model"))) file.create(file.path(file_path, github, "inla.model"))
if(!file.exists(file.path(file_path, github, "inla.model", "dic"))) file.create(file.path(file_path, github, "inla.model", "dic"))
if(!file.exists(file.path(file_path, github, "inla.model", "logfiles"))) file.create(file.path(file_path, github, "inla.model", "logfiles"))
```
...
source(file.path(file_path, gitlab, "scripts/rfunction_inla_my_INLARange.R"))
source(file.path(file_path, gitlab, "scripts/rfunction_inla_my_ggField.R"))
source(file.path(file_path, gitlab, "scripts/rfunction_inla_myINLA_extract_2.R"))
source(file.path(file_path, gitlab, "scripts/rfunction_inla_my_ggMesh.R"))
source(file.path(file_path, gitlab, "scripts/rfunction_sommer_myanova.R"))
source(file.path(file_path, gitlab, "scripts/rfunction_ggsave_my_ggsave_plots.R"))
source(file.path(file_path, gitlab, "scripts/rfunction_busywork.R"))
source(file.path(file_path, gitlab, "scripts/rfunction_inla_my_extract_projection_values.R"))
source(file.path(file_path, gitlab, "scripts/rfunction_latex_subfiguregenerator.R"))
```
...
base_schisto_df<- read.csv(file.path(file_path, github, "schisto_bym_data.csv"))

raster_data <- st_read(file.path(file_path, gitlab, "data/anaplasma_data_with_env_layers_coarsest.gpkg")) #, layer = "cap_with_env"))
...
kruger_shape <- st_read(file.path(file_path, gitlab, "shapefiles/bndry_kruger.shp"))
...
bdry.sp2s <- inla.sp2segment(kruger_shape)
...
my_mesh_function <- function(DT = NULL,
  ...
  # mesh
  # fmesher replaced the inla.mesh functions
...
Max_edge_set <- if(isTRUE(LOCUTMset)){
  #this is inner edge length 1.5km and outer edge length 15km in UTM
  c(5000, 20000)
  #c(10000, 30000)
} else {
...
Cutoff_set <- if(isTRUE(LOCUTMset)){
  c(2000) #UTM 1000 aka 1000 meters or 1km
} else {
...
Offset_set <- if(isTRUE(LOCUTMset)){
  #this is 5km inside and 10km outside the boundary, in UTM
  c(3000, 5000)
} else {
  c(0.045, 0.009)
}
...
save(mod1.list.extract, file = file.path(file_path, github, "inla.model", "mod1.list.extract.R"))
#load(file = file.path(file_path, "github", "inla.model", "mod1.list.extract.R"))
...
Both models ran - the burden model subset out to only 66 individuals, and the WAIC and DIC values appear to be drastically different between the log(CAA +1) and T/C ratio models.
...
save(mod2.list.extract, file = file.path(file_path, github, "inla.model","mod2.list.extract.R"))
...
Both models ran, which is great! It looks like when modeling with log(CAA+1) as the response, BCS is significant, but not when modeling with T/C ratio as the response. 
...
write.csv(mod3.parameters, file.path(file_path, github, "inla.model","dic","mod_table_values.csv"))
...
save(mod3.list.extract, file = file.path(file_path, github, "inla.model","mod3.list.extract.R"))
...
texreg::texreg(mod3.list.extract,
               ...
               file = file.path(file_path, github, "tables", "ch2_table_results_full_mod3.tex"))
...
texreg::texreg(mod3.list.extract[c(1: as.integer(length(mod3.list.extract)/2))],
               ...
               use.packages = FALSE,
               caption.above = TRUE
               ,file = file.path(file_path, github, "tables", "ch2_table_results_full_mod3a.tex")
)
...
texreg::texreg(mod3.list.extract[c((as.integer(length(mod3.list.extract)/2)+1):length(mod3.list.extract))],
               ...
               use.packages = FALSE,
               caption.above = TRUE
               ,file = file.path(file_path, github, "tables", "ch2_table_results_full_mod3b.tex")
)
...
mod3.list.plot <-
  lapply(1:length(resp), function(x){
    ...
    data.table::fwrite(ggField$data, file.path(file_path, github, "inla.model", "dic_fieldplots",
                                                paste0("mod3_",x,"_fieldplot_dt.csv")))
    ggField 
  })
save(mod3.list.plot, file = file.path(file_path, github, "inla.model","mod3_list_ggfield_plot.R"))
...
my_ggsave_plots(plotname = mod3_ggfield,
                plot_path = file.path(file_path, github, "figures"),
                plot_device = "png",
                height = textwidth,
                width = textheight,
                units = "mm")
...
mod3.list.halvingrange.dt <-
  my_INLARange(mod3.list,
               ...
               Resolution = 10000)
#mod3.list.halvingrange.dt
data.table::fwrite(mod3.list.halvingrange.dt, file = file.path(file_path, github, "inla.model", "mod3_list_halvingrange_dt.csv"))
...
mod3.list.halvingrange <-
  ...
  as.data.frame()
mod3.list.halvingrange
write.csv(mod3.list.halvingrange, file = file.path(file_path, github, "inla.model","mod3_halvingrange.csv"))
#mod3.list.halvingrange <- read.csv(file = file.path(file_path, github, "inla.model","mod3_halvingrange.csv"), header = T)
...
data.table::fwrite(as.list(mod.logfile), append = T,
                   file = file.path(file_path, github, "inla.model", "logfiles", "inlarunlog_mod3.txt"))
