...
knp.boundary <- st_read("~eberleyarborough/Documents/2023 Buffalo XS Disease Data/Anaplasma/gitlab/anaplasma_inla/shapefiles/bndry_kruger.shp") # This is in WGS84/UTM 36S

# Section boundaries from VWS
knp.sections <- st_read("~eberleyarborough/Documents/2023 Buffalo XS Disease Data/SANParks KNP Shapefiles/knp_sections") # This is also in WGS84/UTM 36S
...
schisto_data <- read_csv("schisto_data.csv", col_types = cols(animal.ID = col_integer(),`T/C Ratio` = col_number(), `CAA Estimate` = col_number()))
...
anaplasma.data <- st_read("~eberleyarborough/Documents/2023 Buffalo XS Disease Data/Anaplasma/gitlab/anaplasma_inla/data/anaplasma_data_with_env_layers_coarsest.gpkg", layer = "cap_with_env")
...
bym.data <- read.csv("schisto_bym_data.csv")
...
```{r fit-model}
bym.model <- inla(bym.formula, family = "binomial", data = bym.data,
...
posterior_prob_by_section %>%
  ...
  gtsave("posterior_schisto_bym2_values.pdf")
