#########
#' This R script is a variation of the code found in `schisto_inlabru_burden_v1.R`,
#' running the same general analyses, but with Schistosoma spp. burden in terms 
#' of CAA titer values log1p transformed to see how that improves model fit.

#' We may also consider log transforming age (years) and including capture 
#' group by day or herd size here as a random effect.
#########
...
#' We will log1p (log(x+1)) transform CAA titer because preliminary data exploration showed
#' that the data is very right-skewed, so if we want to run Gaussian models,
#' this might be a better way to model the response variable.
schisto.pos.sf <- schisto.pos.sf %>%
  mutate(log1pCAA = log1p(CAA.Estimate))
...
MyVar <- c("animal.ID", "age.years", "age.class", "Sex", "BCS.Average", "log1pCAA",
...
#' We might also want to log transform age, so let's do that in case.
schisto.pos.sf <- schisto.pos.sf %>%
  mutate(log.age = log(age.years))
schisto.pos.df <- schisto.pos.df %>%
  mutate(log.age = log(age.years))
...
#' Let's look again but with log.age
...
#' Plot each continuous covariate against the transformed CAA titer data. 
#' Note: the GAM smoother function family should match the type of model you're 
#' planning on running (e.g., negative binomial, gaussian, etc.)
ggplot(data = schist_long, aes(x=AllX, y = log1pCAA)) +
...
#' Perhaps interaction term with age? Would need to consider incorporating
#' age class instead?
...
#' Let log1pCAA_i be the log1p transformed CAA titer value at site i.
#' We assume a normal distribution for log1pCAA_i:

#'   log1pCAA_i ~ N(mu_i, sigma^2)
#'   E[log1pCAA_i]   = mu_i
#'   var[log1pCAA_i] = sigma^2

#' **Model 1**
#' We model the mean mu_i as a function of the host demographics, environmental
#' covariates, and spatial dependency. Note that we're including log transformed
#' age here.

#' mu_i = Intercept + log.age + BCS.Average + TWI_i + EVI_i + MSI_i + soil_i +
#'        firedistance_i + u_i
...
#' is unlikely that the sigma is larger than  3.5. For our range prior, this is
#' stating that we think it is most likely that the range is less than 50km.
Matern <- inla.spde2.pcmatern(mesh1,
                              prior.range = c(50, 0.95),
                              prior.sigma = c(3.5, 0.05))

#' Then there is also the observation noise. That is the sigma in:
#' log1pCAA_i ~ N(mu_i, sigma^2). We will also use the same PC prior:
#' P(sigma >3.5) = 0.05 
PCPrior.Sigma <- list(prec = list(prior = "pc.prec", 
                                  param = c(3.5, 0.05)))

#' When including a random effect, you introduce a random intercept into the 
#' model equation for each observation of that effect. These random intercepts
#' are assumed to be independent and identically, normally distributed, with a
#' mean of 0 and a variance of sigma_effect^2. As such, we need to specify a 
#' a prior for each random effect in the model. A reasonable choice is a PC 
#' prior, as well - the one we've used previously. So for our herd size effect,
#' we would have Pr(sigma_h > std.dev_log1pCAA) = alpha, or:
PC.prior.Sigma.h <- list(prec = list(prior = "pc.prec", 
                                     param = c(3.5, 0.05)))

#' Step 2: Define components
#' Models 1 and 2: LM **without** spatial dependency, **with and without** a random
#' effect for herd size.

#' You can put anything you think you *might* need or want in your components
#' list, but you don't have to include them all in your likelihood. In this case,
#' since we'll be fitting a model with and without the random effect of herd size,
#' we'll include it in the components here, but only specify it in the 
...
cmp1 <- ~ Intercept(1) +
  ...
  fire(weighted_dist_to_fire.std,
       model = "linear") +
  herd(approx.herd.size,
       model = "iid", #' stands for independent and identically distributed
       hyper = PC.prior.Sigma.h) #' call the PC prior specified above
...
#' Models 3 and 4: LM **with** spatial dependency, **with and without** a random
#' effect for herd size
...
#' Model 1: LM **without** spatial dependency, **without** herd size
#' Note that the `bru_obs()` command comes from the `inlabru` package
lik1 <- bru_obs(log1pCAA ~ Intercept + Age + BCS + TWI + MSI + EVI + soil + 
                fire,
                family = "gaussian",
                data = schisto.pos.sf)

#' Model 2: LM **without** spatial dependency, **with** herd size
#' Note that the `bru_obs()` command comes from the `inlabru` package
lik2 <- bru_obs(log1pCAA ~ Intercept + Age + BCS + TWI + MSI + EVI + soil + 
                fire + herd,
                family = "gaussian",
                data = schisto.pos.sf)

#' Model 3: LM **with** spatial dependency, **without** herd size
lik3 <- bru_obs(log1pCAA ~ Intercept + Age + BCS + TWI + MSI + EVI + soil + 
                fire + SRF,
                family = "gaussian",
                data = schisto.pos.sf)

#' Model 4: LM **with** spatial dependency, **with** herd size
lik4 <- bru_obs(log1pCAA ~ Intercept + Age + BCS + TWI + MSI + EVI + soil + 
                  fire + herd + SRF,
                family = "gaussian",
                data = schisto.pos.sf)
...
#' extra output info you don't want. We also will tell R to include our PC
#' prior for observational noise here.
#' This will be the same across models
Options <- list(quantiles =  c(0.025, 0.975),
                control.compute = list(dic = TRUE, waic = TRUE),
                control.family = list(hyper = PCPrior.Sigma))
...
#' Age appears to be important in terms of credible intervals, but that's all
#' (negative effect).
#' Precision for observation noise = 0.329 (we would need to transform this to
#' get the actual sigma value).

#' Model 2: LM **without** spatial dependency, **with** herd size
mod2 <- bru(components = cmp1, lik2, options = Options)

mod2$summary.fixed #' slight changes compared to mod 1, but nothing major
mod2$summary.hyperpar
#' precision for the observation noise is still 0.329, and precision for herd is
#' 152.405
#' for random effects, you run the command:
mod2$summary.random$herd
#' none of the herd size values appear to be important in terms of credible
#' intervals.

#' Model 3: LM **with** spatial dependency, **without** herd size
mod3 <- bru(components = cmp2, lik3, options = Options)

mod3$summary.fixed #' again, similar results compared to the previous 2 models
mod3$summary.hyperpar

#' The precision for the observation noise is not 0.341.
#' The range for the SRF is 3.673, and the st dev for the SRF is 0.493

#' Model 4: LM **with** spatial dependency, **with** herd size
mod4 <- bru(components = cmp2, lik4, options = Options)

mod4$summary.fixed #' no huge changes here
mod4$summary.hyperpar
#' Precision for observation noise still 0.341
#' Precision for herd = 162.4
#' Range for SRF = 4.111
#' Std dev for SRF = 0.489
mod4$summary.random$herd
#' Herd size non-important, not surprisingly.
...
#* Subsection 8.1: Model 1 - LM **without** spatial dependency, **without** herd size -----
...
Sigma1
#' 1.75609, consistent with our prior stating that it was likely that the sigma
#' was < 3.5
...
E1sqr <- createDHARMa(simulatedResponse = Ysim1,
                      observedResponse = schisto.pos.sf$log1pCAA,
...
#' This fit is a MUCH improved compared to when we didn't transform CAA titers.

#' Step 3: Plot the residuals vs the covariates using `DHARMa`
plotResiduals(E1sqr, form = schisto.pos.sf$log.age) #' Still some problems here, but looking better
plotResiduals(E1sqr, form = schisto.pos.sf$BCS.Average) #' Much better
plotResiduals(E1sqr, form = schisto.pos.sf$topo_wetness_index.std) #' Good
plotResiduals(E1sqr, form = schisto.pos.sf$clay_percent_30cm.std) #' Better, but still problems
plotResiduals(E1sqr, form = schisto.pos.sf$jan_apr_23_evi.std) #' Better, but still problems
plotResiduals(E1sqr, form = schisto.pos.sf$jan_apr_23_msi.std) #' Good
plotResiduals(E1sqr, form = schisto.pos.sf$weighted_dist_to_fire) #' Good
#' Overall, a huge improvement compared to the raw CAA values!

#' Lets also look at the deviance residuals (log1pCAA_i - Fitted_i) / sigma using
#' the `residuals()` function from the `stats` package.
schisto.pos.sf$deviance.mod1 <- residuals(mod1)$deviance.residuals

#' We can also calculate the ordinary residuals, which are just log1pCAA_i - Fitted_i
#' for a Gaussian model.
schisto.pos.sf$resid.mod1 <-  schisto.pos.sf$log1pCAA - schisto.pos.sf$Pi.mod1 #' obs - fitted
...
pResidNorm <- ggplot(data = schisto.pos.sf, aes(x = resid.mod1)) + 
  ...
  theme(text = element_text(size=15))
pResidNorm
#' This still looks a bit right-skewed, but overall, a big improvement.

#' Based on the scaled quantile residuals, we can probably assume that the plots
#' of residuals vs covariates will have some issues, but let's just see one or two.

#' Plot residuals vs. age.years
pResidvsAge <- ggplot(data = schisto.pos.sf, aes(y = resid.mod1, x = log.age)) + 
  geom_point(shape = 1, size = 1) +
  geom_smooth() +
...
#' Plot residuls vs. MSI
pResidvsMSI <- ggplot(data = schisto.pos.sf, aes(y = resid.mod1, x = jan_apr_23_msi.std)) + 
  geom_point(shape = 1, size = 1) +
  geom_smooth() +
  labs(x = "MSI (std)", y = "Residuals") +
  theme(text = element_text(size=15)) +
  geom_hline(yintercept = 0, lty = 2)
pResidvsMSI
...
#' Plot it
m1residspat <- ggplot(data = kruger.km) +
               geom_sf(fill = "transparent") +
               geom_sf(data = schisto.pos.sf,
                       aes(col = MyCol,
                            size = MySize),
...
#' And plot
mod1variogram <- ggplot(data = V1, aes(x = dist, y = gamma)) +
                 geom_point() +
...
#* Subsection 8.2: Model 3 - LM **with** spatial dependency, **without** herd size ----- 

#' Step 1: Get scaled quantile residuals
#' To do this, we first need to get the fitted values from the model
N <- nrow(schisto.pos.sf)
schisto.pos.sf$Pi.mod3 <- mod3$summary.fitted.values[1:N, "mean"]

#' And we need the sigma for the "family noise" or the residual standard
#' deviation. Remember that we can't simply back transform the precision (tau)
#' to sigma directly.
tau3     <- mod3$marginals.hyperpar$`Precision for the Gaussian observations`
MySqrt  <- function(x) { 1 / sqrt(x) }
Sigma3   <- inla.emarginal(MySqrt, tau3)
Sigma3
#' 1.7282, a bit less than without the SRF.

#' Start a loop simulating 1000 gaussian distributed observations in the 
#' response variable, for each  given fitted value, Pi.mod1. We'll use the 
#' `rnorm()` function from the `stats` package. Note: gaussian simulations
#' require the mean fitted values and residual standard deviation.
Ysim3 <- matrix(nrow = N, ncol = 1000)
for (i in 1:1000){
  Ysim3[,i] <- rnorm(n = N,
                     mean = schisto.pos.sf$Pi.mod3,
                     sd = Sigma3)
}
...
E3sqr <- createDHARMa(simulatedResponse = Ysim3,
                      observedResponse = schisto.pos.sf$log1pCAA,
                      fittedPredictedResponse = schisto.pos.sf$Pi.mod3,
                      integerResponse = FALSE) #' is "TRUE" for Bernoulli and poisson data
...
plotQQunif(E3sqr,
           ...
           testOutliers = TRUE,
           testDispersion = FALSE)
#' Better than with raw CAA values; however, we have slight deviation here (p = 0.04498)

#' Step 3: Plot the residuals vs the covariates using `DHARMa`
plotResiduals(E3sqr, form = schisto.pos.sf$log.age) #' Good, better than mod 2
plotResiduals(E3sqr, form = schisto.pos.sf$BCS.Average) #' Good
plotResiduals(E3sqr, form = schisto.pos.sf$topo_wetness_index.std) #' Good
plotResiduals(E3sqr, form = schisto.pos.sf$clay_percent_30cm.std) #' Good - better than without SRF
plotResiduals(E3sqr, form = schisto.pos.sf$jan_apr_23_evi.std) #' Good - better than without SRF
plotResiduals(E3sqr, form = schisto.pos.sf$jan_apr_23_msi.std) #' Good
plotResiduals(E3sqr, form = schisto.pos.sf$weighted_dist_to_fire) #' Good
#' Overall, much better than with raw CAA values, and better when compared to mod 1.

#' Lets also look at the deviance residuals (CAA_i - Fitted_i) / sigma using
#' the `residuals()` function from the `stats` package.
schisto.pos.sf$deviance.mod3 <- residuals(mod3)$deviance.residuals

#' We can also calculate the ordinary residuals, which are just CAA_i - Fitted_i
#' for a Gaussian model.
schisto.pos.sf$resid.mod3 <-  schisto.pos.sf$log1pCAA - schisto.pos.sf$Pi.mod3 #' obs - fitted

#' Now we can check for homogeneity of variance plotting the ordinary residuals
#' vs. fitted values
pResidvsFit <- ggplot(data = schisto.pos.sf, aes(y = resid.mod3, x = Pi.mod3)) + 
  geom_point(shape = 1, size = 1) +
  labs(x = "Fitted values", y = "Residuals") +
  theme(text = element_text(size=15)) +
  geom_hline(yintercept = 0, lty = 2)
pResidvsFit
#' Might have some issues, but much better than the model with raw CAA values.

#' And we can check the normality of the ordinary residuals
pResidNorm <- ggplot(data = schisto.pos.sf, aes(x = resid.mod3)) + 
  geom_histogram(alpha = 0.5, color = "darkblue", fill = "lightblue") +
  labs(y = "Histogram", x = "Residuals") +
  theme(text = element_text(size=15))
pResidNorm
#' Looking more normal than model 1, but still a bit of a right-skew.

#' Based on the scaled quantile residuals, we can probably assume that the plots
#' of residuals vs covariates will be ok, but let's just see one or two.

#' Plot residuals vs. age.years
pResidvsAge <- ggplot(data = schisto.pos.sf, aes(y = resid.mod3, x = log.age)) + 
  geom_point(shape = 1, size = 1) +
  geom_smooth() +
  labs(x = "Age (years)", y = "Residuals") +
  theme(text = element_text(size=15)) +
  geom_hline(yintercept = 0, lty = 2)
pResidvsAge
#' Not ideal, but looks similar to model 1 - possible non-linear patterns here?

#' Plot residuals vs. EVI
pResidvsEVI <- ggplot(data = schisto.pos.sf, aes(y = resid.mod3, x = jan_apr_23_evi.std)) + 
  geom_point(shape = 1, size = 1) +
  geom_smooth() +
  labs(x = "EVI (std)", y = "Residuals") +
  theme(text = element_text(size=15)) +
...
#' Plot residuls vs. MSI
pResidvsMSI <- ggplot(data = schisto.pos.sf, aes(y = resid.mod3, x = jan_apr_23_msi.std)) + 
  geom_point(shape = 1, size = 1) +
  geom_smooth() +
  labs(x = "MSI (std)", y = "Residuals") +
  theme(text = element_text(size=15)) +
...
#' Remember that scaled quantile residuals are centered on 0.5, while Pearson 
#' residuals are centered around 0.
schisto.pos.sf$MyCol3 <- ifelse(schisto.pos.sf$resid.mod3 >= 0, "red", "blue")
schisto.pos.sf$MySize3 <- rescale(abs(schisto.pos.sf$resid.mod3), to = c(0,10))

#' Plot it
m3residspat <- ggplot(data = kruger.km) +
               geom_sf(fill = "transparent") +
               geom_sf(data = schisto.pos.sf,
                       aes(col = MyCol3,
                           size = MySize3),
...
m3residspat
#' Hmm, hard to tell if there's improvement between models 1 and 3?

#' Option 3: Make a variogram
#' First, make a dataframe with the residuals and capture locations
mod3data <- data.frame(E1 = schisto.pos.sf$resid.mod3,
                       Xkm = schisto.pos.sf$Xkm,
                       Ykm = schisto.pos.sf$Ykm)
...
#' And plot
mod3variogram <- ggplot(data = V3, aes(x = dist, y = gamma)) +
               geom_point() +
               geom_smooth(se = FALSE, span = 1) +
               labs(x = "Distance (km)", y = "Semi-variogram Mod 3") +
               theme(text = element_text(size = 15))
mod3variogram
#' This looks the same as the one without the SRF.
...
DICs  <- c(mod1$dic$dic, mod2$dic$dic, mod3$dic$dic, mod4$dic$dic)
WAICs <- c(mod1$waic$waic, mod2$waic$waic, mod3$waic$waic, mod4$waic$waic)
...
#' We can also put the residual plots together in a multipanel figure for ease
residspatplots <- plot_grid(m1residspat, m3residspat, ncol = 2)
residspatplots

variogramplots <- plot_grid(mod1variogram, mod3variogram, ncol = 1)
variogramplots

#' Based off of this, there's really no difference between any of the models.
#' Regardless, we should throw out the herd size as a random effect, but in terms
#' of including the SRF vs not, there isn't a huge effect on burden output. 

# Section 10: Plotting the SRF -----

#' Posterior distribution of the range.
Range     <- spde.posterior(mod3, "SRF", what = "range")
plot(Range)
#' Suuuuuper small range here.

#' This is the imposed spatial dependence.
MaternCor <- spde.posterior(mod3, "SRF", what = "matern.correlation")
pB <- plot(MaternCor) +
  labs(x = "Distance (km)", y = "Correlation") +
  coord_cartesian(ylim = c(0, 1))
pB
#' Spatial autocorrelation decay is rapid.
...
p.Matern <- pB +
  ...
  theme(text = element_text(size = 15)) 
p.Matern
#' This says that the spatial autocorrelation drops below 0.8 at ~0.5km, below
#' 0.5 at ~1km, and below 0.1 at ~2.6km in this model - crazy!
...
#' Recall that the output of the figure is information in log1pCAA_i that **cannot**
#' be explained by the covariates and is spatial in nature! 
#' This map indicates that we're overfitting the model.
