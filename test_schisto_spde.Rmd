...
# Set global vars
LOCUTMset = FALSE # because coordinates are already in UTM
toUTMset = FALSE
...
Cutoff_set <- if(isTRUE(LOCUTMset)){
  c(1000) #UTM 1000 aka 1000 meters or 1km
} else {
...
mod3.list.plot <-
  lapply(1:length(resp), function(x){
    ...
    my_ggField(mod3.list[[x]], Mesh = mod3.mesh.list[[x]], Str_name = "Range for ", Variable = "mean",
      ...
      theme_classic() +
      my_plot_settings
    ggField <- ggField + geom_sf(data = bdry.utm, inherit.aes = FALSE, fill = NA, color = "black", linewidth = 1) 
    data.table::fwrite(ggField$data, file.path(file_path, "inla.model", "dic_fieldplots",
